...
import static com.example.projetjee.model.dao.CourseDAO.getCoursesBySubjectId;
...
import static com.example.projetjee.model.dao.SubjectDAO.getSubjectsByStudentId;
...
public class GeneratorPdfGradeReport {
    ...
    public Map<String, Map<String, List<Grade>>> fetchSubjectCourseGrades(int userId) {
        ...
        List<Subjects> subjects = getSubjectsByStudentId(userId);

        if (subjects != null) {
            for (Subjects subject : subjects) {
                Map<String, List<Grade>> courseGradesMap = new LinkedHashMap<>();
                List<Course> courses = getCoursesBySubjectId(subject.getSubjectId());

                for (Course course : courses) {

                    List<Grade> grades = getGradeByStudentId(userId).stream()
